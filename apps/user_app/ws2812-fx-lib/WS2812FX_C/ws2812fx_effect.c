...
u16 colorful_lights_auto(void)
{
    ...
    { // 渐变模式
        ...
        u32 color = WS2812FX_color_blend(cur_colors, dest_colors, (u8)brightness);
        Adafruit_NeoPixel_fill(color, _seg->start, _seg_len);
        ...
        if (_seg_rt->counter_mode_step > 255)
        {
            _seg_rt->counter_mode_step = 0;
            _seg_rt->aux_param += 1;
            if (_seg_rt->aux_param > _seg->c_n - 2) //
            {
                _seg_rt->aux_param = 0;
                ...
                if (brightness > 255) // 如果已经达到最大亮度，开始渐灭
                {
                    ...
                    if (brightness < next_led_begin_threshold_val)
                    {
                        ...
                        if (0 != next_led_index &&                               /* 不是第0个灯 */
                            0 == _seg_rt->led_index_enable_buff[next_led_index]) /* 下一个灯没有被使能 */
                        {
                            _seg_rt->led_index_enable_buff[next_led_index] = 1;
                        }
                    }
                }
